        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <span className="inline-block px-4 py-2 bg-[#12AAD1]/10 text-[#12AAD1] text-sm font-semibold rounded-full mb-6">Graphic Design</span>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[#0B1214] leading-tight mb-6">Stunning <span className="text-[#12AAD1]">Visual</span> Designs</h1>
              <p className="text-lg text-[#A3B0B7] mb-8">Creative design solutions that capture your brand essence and leave lasting impressions on your audience.</p>
              <Link to="/contact" className="btn-primary">Start Your Project <ArrowRight className="w-5 h-5" /></Link>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
              <img loading="lazy" src="/portfolio-branding.jpg" alt="Graphic Design" className="rounded-2xl shadow-2xl" />
            </motion.div>
          </div>
        </div>
      </section>
