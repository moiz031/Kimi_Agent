        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <span className="inline-block px-4 py-2 bg-[#12AAD1]/10 text-[#12AAD1] text-sm font-semibold rounded-full mb-6">SEO Services</span>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[#0B1214] leading-tight mb-6">Dominate <span className="text-[#12AAD1]">Search Rankings</span></h1>
              <p className="text-lg text-[#A3B0B7] mb-8">Data-driven SEO strategies that improve your visibility, drive organic traffic, and generate qualified leads for your business.</p>
              <Link to="/contact" className="btn-primary">Get Your SEO Audit <ArrowRight className="w-5 h-5" /></Link>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
              <img loading="lazy" src="/portfolio-seo.jpg" alt="SEO Services" className="rounded-2xl shadow-2xl" />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding section-surface">
        <div className="container-custom">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <span className="inline-block px-4 py-2 bg-[#12AAD1]/10 text-[#12AAD1] text-sm font-semibold rounded-full mb-4">Our SEO Services</span>
